...
`;function mt(t){const e=/^cv\s+(\d+)$/.exec((t??"").trim());return e?parseInt(e[1],10):-1}function ut(t){const e=Number(t);return isNaN(e)?String(t):e.toFixed(2)}window.customCards=window.customCards||[],window.customCards.push({type:"miraie-ac-card-in",name:"MirAIe AC Card",description:"A premium thermostat card for Panasonic MirAIe AC units",preview:!0});let gt=class extends ot{constructor(){super(...arguments),this._openPanel=null}static get styles(){return pt}static getConfigForm(){return{schema:[{name:"entity",required:!0,selector:{entity:{domain:"climate"}}},{name:"name",selector:{text:{}}},{name:"accent_color",selector:{ui_color:{}}},{name:"",type:"expandable",title:"Display Sensors",icon:"mdi:thermometer",schema:[{name:"room_temp_sensor",selector:{entity:{domain:"sensor"}}},{name:"humidity_sensor",selector:{entity:{domain:"sensor"}}}]},{name:"",type:"expandable",title:"Convertible & Controls",icon:"mdi:toggle-switch-outline",schema:[{name:"convertible_mode_entity",selector:{entity:{domain:"select"}}},{name:"nanoe_switch",selector:{entity:{domain:"switch"}}},{name:"display_switch",selector:{entity:{domain:"switch"}}},{name:"coil_clean_button",selector:{entity:{domain:"button"}}},{name:"coil_cleaning_sensor",selector:{entity:{domain:"binary_sensor"}}},{name:"filter_alert_sensor",selector:{entity:{domain:"binary_sensor"}}}]},{name:"",type:"expandable",title:"Diagnostics & Energy",icon:"mdi:chart-line",schema:[{name:"rssi_sensor",selector:{entity:{domain:"sensor"}}},{name:"energy_today_sensor",selector:{entity:{domain:"sensor"}}},{name:"energy_yesterday_sensor",selector:{entity:{domain:"sensor"}}}]}]}}static getStubConfig(){return{type:"custom:miraie-ac-card-in",entity:""}}setConfig(t){if(!t.entity||!t.entity.startsWith("climate."))throw new Error("Please define a valid climate entity.");this._config={...t},this._openPanel=null}shouldUpdate(t){if(t.has("_config")||t.has("_openPanel"))return!0;if(t.has("hass")&&this._config){const e=t.get("hass");if(!e)return!0;const i=this._config;return[i.entity,i.room_temp_sensor,i.humidity_sensor,i.convertible_mode_entity,i.nanoe_switch,i.display_switch,i.coil_clean_button,i.coil_cleaning_sensor,i.filter_alert_sensor,i.rssi_sensor,i.energy_today_sensor,i.energy_yesterday_sensor].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}return!1}render(){if(!this.hass||!this._config)return null;const t=this._config,e=this.hass.states[t.entity];if(!e)return B`<ha-card><div class="error">Entity not found: ${t.entity}</div></ha-card>`;const i=e.attributes,s="unavailable"!==e.state&&"unknown"!==e.state,n="off"!==e.state&&s,r=t.name||i.friendly_name||"AC",o=i.temperature,a=i.min_temp??16,c=i.max_temp??30,l=e.state,d=i.fan_mode,h=i.swing_mode,p=i.swing_horizontal_mode,m=i.preset_mode,u=t.room_temp_sensor?this.hass.states[t.room_temp_sensor]:void 0,g=u?u.state:i.current_temperature,_=t.humidity_sensor?this.hass.states[t.humidity_sensor]:void 0,v=t.convertible_mode_entity?this.hass.states[t.convertible_mode_entity]:void 0,f=t.nanoe_switch?this.hass.states[t.nanoe_switch]:void 0,y=t.display_switch?this.hass.states[t.display_switch]:void 0,b=t.coil_clean_button?this.hass.states[t.coil_clean_button]:void 0,$=t.coil_cleaning_sensor?this.hass.states[t.coil_cleaning_sensor]:void 0,x=t.filter_alert_sensor?this.hass.states[t.filter_alert_sensor]:void 0,w=t.rssi_sensor?this.hass.states[t.rssi_sensor]:void 0,A=t.energy_today_sensor?this.hass.states[t.energy_today_sensor]:void 0,S=t.energy_yesterday_sensor?this.hass.states[t.energy_yesterday_sensor]:void 0;let k=[],E="cv 0",C=!1;v?(k=v.attributes?.options??[],E=v.state??"cv 0"):i.preset_modes&&i.preset_modes.some(t=>t.startsWith("cv "))&&(k=i.preset_modes.filter(t=>t.startsWith("cv ")),k.includes("cv 0")||k.push("cv 0"),E=i.preset_mode?.startsWith("cv ")?i.preset_mode:"cv 0",C=!0);const P=k.filter(t=>mt(t)>0).sort((t,e)=>mt(t)-mt(e)),z=["cv 0",...P],O=z.indexOf(E),U=(M=k,M?.length?M.includes("cv 60")&&M.includes("cv 50")?"Converti8":"Converti7":"Convertible");var M;const H=P.length>0?O/(z.length-1)*100:0;let N="";if(t.accent_color)if(Array.isArray(t.accent_color))N=`rgb(${t.accent_color.join(",")})`;else if("string"==typeof t.accent_color){const e=t.accent_color.toLowerCase();N="primary"===e?"var(--primary-color)":"accent"===e?"var(--accent-color)":/^[a-z]+$/.test(e)?`var(--${e}-color, ${e})`:e}return B`
      <ha-card style="${N?`--miraie-accent: ${N};`:""}">
        ...
        <div class="temp-block">
          <button
            class="temp-btn"
            ?disabled=${!n||"fan_only"===l||null!=o&&Number(o)<=Number(a)}
          ...
          <button
            class="temp-btn"
            ?disabled=${!n||"fan_only"===l||null!=o&&Number(o)>=Number(c)}
          ...
          <div class="pills">
            ${["none","eco","boost"].map(t=>B`
              <button
                ...
                ?disabled=${!n||["dry","auto","fan_only"].includes(l)&&"none"!==t}
          ...
          <div class="section" style="${["dry","auto","fan_only"].includes(l)?"opacity: 0.5; pointer-events: none;":""}">
            ...
            <div class="step-slider-wrap">
                ...
                <div class="step-notches">
                  ${z.map((e,i)=>B`
                    <button
                      ...
                      ?disabled=${!n||["dry","auto","fan_only"].includes(l)}
                      @click=${()=>C?this._setPreset(e):this._selectOption(t.convertible_mode_entity,e)}
                    ></button>
                  `)}
                </div>
              </div>
              ...
              <div class="step-labels">
                ...
                <span class="step-label">${mt(P[P.length-1])}%</span>
              </div>
            </div>
          </div>
        `:""}
