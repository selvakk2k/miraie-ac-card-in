      ...
      <ha-card style="${c}" class="gh-full-card">
        ...
        <div class="gh-select-container">
          ...
          <div class="gh-select-wrapper" style="${i&&"fan_only"!==r?"":"opacity: 0.5; pointer-events: none;"}">
            ...
            ${"preset"===this._ghDropdown?F`
              <div class="gh-dropdown-menu">
                ${h.map(e=>{const i=t.attributes.preset_mode===e||"none"===e&&(!t.attributes.preset_mode||/^cv[\s_]/.test(t.attributes.preset_mode));return F`
          ...
          ${u.length>0?F`
            <div class="gh-select-wrapper" style="${!i||["dry","auto","fan_only"].includes(r)?"opacity: 0.5; pointer-events: none;":""}">
              ...
              ${"cv"===this._ghDropdown?F`
                <div class="gh-dropdown-menu">
                  ${u.map(e=>{const i=mt(e),s=t.attributes.preset_mode===e||100===i&&(!t.attributes.preset_mode||!/^cv[\s_]/.test(t.attributes.preset_mode));return F`
        ...
        ${g||v||_||b||y?F`
          <div class="gh-extra-chips">
            ${g?F`<div class="gh-chip ${"on"===g.state?"active":""}" @click=${()=>this._toggleSwitch(l.nanoe_switch,g.state)}><ha-icon icon="mdi:virus-outline"></ha-icon>Nanoe</div>`:""}
            ${v?F`<div class="gh-chip ${"on"===v.state?"active":""}" @click=${()=>this._toggleSwitch(l.display_switch,v.state)}><ha-icon icon="mdi:lightbulb-outline"></ha-icon>Display</div>`:""}
            ${_||f?F`<div class="gh-chip ${"on"===f?.state?"active":""}" @click=${()=>_?this._pressButton(l.coil_clean_button):null}><ha-icon icon="mdi:spray"></ha-icon>${"on"===f?.state?"Cleaning...":"Clean Coil"}</div>`:""}
            ${b?F`<div class="gh-chip-text"><ha-icon icon="mdi:lightning-bolt"></ha-icon>Today: ${ut(b.state)} kWh</div>`:""}
            ${y?F`<div class="gh-chip-text"><ha-icon icon="mdi:lightning-bolt"></ha-icon>Yesterday: ${ut(y.state)} kWh</div>`:""}
          </div>
        `:""}
        
        ${$?F`
          <div class="gh-footer-text">
            <ha-icon icon="mdi:wifi"></ha-icon> ${$.state} dBm
          </div>
        `:""}
      </ha-card>
