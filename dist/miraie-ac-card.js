...
`;function mt(t){const e=/^cv[\s_]+(\d+)$/.exec((t??"").trim());return e?parseInt(e[1],10):-1}function ut(t){const e=Number(t);return isNaN(e)?String(t):e.toFixed(2)}window.customCards=window.customCards||[],window.customCards.push({type:"miraie-ac-card-in",name:"MirAIe AC Card",description:"A premium thermostat card for Panasonic MirAIe AC units",preview:!0});let gt=class extends rt{constructor(){super(...arguments),this._openPanel=null,this._expanded=!1,this._ghDropdown=null,this._handleWindowClick=()=>{this._ghDropdown&&(this._ghDropdown=null)}}static get styles(){return ht}connectedCallback(){super.connectedCallback(),window.addEventListener("click",this._handleWindowClick)}disconnectedCallback(){window.removeEventListener("click",this._handleWindowClick),super.disconnectedCallback()}static getConfigForm(){return{schema:[{name:"entity",required:!0,selector:{entity:{domain:"climate"}}},{name:"name",selector:{text:{}}},{name:"theme",selector:{select:{options:[{label:"Default HA Theme",value:"default"},{label:"Material You",value:"material_you"}]}}},{name:"layout",selector:{select:{options:[{label:"Default (Full)",value:"default"},{label:"Compact (Expandable)",value:"compact"}]}}},{name:"full_layout",selector:{select:{options:[{label:"Classic",value:"default"},{label:"Google Home",value:"google_home"}]}}},{name:"accent_color",selector:{ui_color:{}}},{name:"main_color",selector:{ui_color:{}}},{name:"",type:"expandable",title:"Display Sensors",icon:"mdi:thermometer",schema:[{name:"room_temp_sensor",selector:{entity:{domain:"sensor"}}},{name:"humidity_sensor",selector:{entity:{domain:"sensor"}}}]},{name:"",type:"expandable",title:"Convertible & Controls",icon:"mdi:toggle-switch-outline",schema:[{name:"nanoe_switch",selector:{entity:{domain:"switch"}}},{name:"display_switch",selector:{entity:{domain:"switch"}}},{name:"coil_clean_button",selector:{entity:{domain:"button"}}},{name:"coil_cleaning_sensor",selector:{entity:{domain:"binary_sensor"}}},{name:"filter_alert_sensor",selector:{entity:{domain:"binary_sensor"}}}]},{name:"",type:"expandable",title:"Diagnostics & Energy",icon:"mdi:chart-line",schema:[{name:"rssi_sensor",selector:{entity:{domain:"sensor"}}},{name:"energy_today_sensor",selector:{entity:{domain:"sensor"}}},{name:"energy_yesterday_sensor",selector:{entity:{domain:"sensor"}}}]}]}}static getStubConfig(){return{type:"custom:miraie-ac-card-in",entity:""}}setConfig(t){if(!t.entity||!t.entity.startsWith("climate."))throw new Error("Please define a valid climate entity.");this._config={...t},this._openPanel=null}updated(t){if(super.updated(t),t.has("_config")){const t=this._config?.theme||"default";this.getAttribute("theme")!==t&&this.setAttribute("theme",t)}}shouldUpdate(t){if(t.has("_config")||t.has("_openPanel")||t.has("_expanded")||t.has("_ghDropdown"))return!0;if(t.has("hass")&&this._config){const e=t.get("hass");if(!e)return!0;const i=this._config;return[i.entity,i.room_temp_sensor,i.humidity_sensor,i.nanoe_switch,i.display_switch,i.coil_clean_button,i.coil_cleaning_sensor,i.filter_alert_sensor,i.rssi_sensor,i.energy_today_sensor,i.energy_yesterday_sensor].filter(Boolean).some(t=>e.states[t]!==this.hass.states[t])}return!1}render(){if(!this.hass||!this._config)return null;const t=this._config,e=this.hass.states[t.entity];if(!e)return F`<ha-card><div class="error">Entity not found: ${t.entity}</div></ha-card>`;const i=e.attributes,s="unavailable"!==e.state&&"unknown"!==e.state,o="off"!==e.state&&s,n=t.name||i.friendly_name||"AC",r=i.temperature,a=i.min_temp??16,c=i.max_temp??30,l=e.state,d=i.fan_mode,p=i.swing_mode,h=i.swing_horizontal_mode,m=i.preset_mode,u=t.room_temp_sensor?this.hass.states[t.room_temp_sensor]:void 0;let g=u?u.state:i.current_temperature;null==g||isNaN(Number(g))||(g=Number(g).toFixed(1));const v=t.humidity_sensor?this.hass.states[t.humidity_sensor]:void 0;let _=v?v.state:void 0;null==_||isNaN(Number(_))||(_=Number(_).toFixed(1));const f=t.nanoe_switch?this.hass.states[t.nanoe_switch]:void 0,b=t.display_switch?this.hass.states[t.display_switch]:void 0,y=t.coil_clean_button?this.hass.states[t.coil_clean_button]:void 0,$=t.coil_cleaning_sensor?this.hass.states[t.coil_cleaning_sensor]:void 0,x=t.filter_alert_sensor?this.hass.states[t.filter_alert_sensor]:void 0,w=t.rssi_sensor?this.hass.states[t.rssi_sensor]:void 0,A=t.energy_today_sensor?this.hass.states[t.energy_today_sensor]:void 0,k=t.energy_yesterday_sensor?this.hass.states[t.energy_yesterday_sensor]:void 0,C="on"===$?.state;let S=[],E="cv_";i.preset_modes&&i.preset_modes.some(t=>/^cv[\s_]/.test(t))&&(S=i.preset_modes.filter(t=>/^cv[\s_]/.test(t)),E=S[0].substring(0,3),S.includes(`${E}0`)||S.push(`${E}0`));let P=i.preset_mode&&/^cv[\s_]/.test(i.preset_mode)?i.preset_mode:`${E}0`;const z=S.filter(t=>mt(t)>0).sort((t,e)=>mt(t)-mt(e)),N=[`${E}0`,...z],j=N.indexOf(P),U=(T=S,T?.length?T.some(t=>60===mt(t))&&T.some(t=>50===mt(t))?"Converti8":"Converti7":"Convertible");var T;const M=z.length>0?j/(N.length-1)*100:0;let D="";if(this._config.accent_color)if(Array.isArray(this._config.accent_color))D=`rgb(${this._config.accent_color.join(",")})`;else if("string"==typeof this._config.accent_color){const t=this._config.accent_color.toLowerCase();D="primary"===t?"var(--primary-color)":"accent"===t?"var(--accent-color)":/^[a-z-]+$/.test(t)?`var(--${t}-color, ${t})`:t}let H="";if(this._config.main_color)if(Array.isArray(this._config.main_color))H=`rgb(${this._config.main_color.join(",")})`;else if("string"==typeof this._config.main_color){const t=this._config.main_color.toLowerCase();H="primary"===t?"var(--primary-color)":"accent"===t?"var(--accent-color)":/^[a-z-]+$/.test(t)?`var(--${t}-color, ${t})`:t}const O=`${D?`--miraie-accent: ${D}; `:""}${H?`--m-bg: ${H}; `:""}`;if("compact"===t.layout&&!this._expanded)return this._renderCompact(e,n,o,r,g,l,a,c,O);if("google_home"===t.full_layout)return this._renderGoogleHomeFull(e,n,o,r,g,l,a,c,O);let R=[];if(o){if(R.push(this._modeLabel(l)),m&&"none"!==m)if(/^cv[\s_]/.test(m)){const t=mt(m);R.push(0===t?"Normal Limit":t+"% Limit")}else R.push(m.charAt(0).toUpperCase()+m.slice(1));R.push(`Fan: ${d??"Auto"}`)}return F`
      <ha-card style="${O}">
        ...
        <div class="temp-block">
          <button
            class="temp-btn"
            ?disabled=${!o||"fan_only"===l||null!=r&&Number(r)<=Number(a)||C}
            @click=${()=>this._adjustTemp(-1,r,a)}
          >
          ...
          <button
            class="temp-btn"
            ?disabled=${!o||"fan_only"===l||null!=r&&Number(r)>=Number(c)||C}
            @click=${()=>this._adjustTemp(1,r,c)}
          >
        ...
        <div class="section">
          ...
          <div class="pills" style="${C?"opacity: 0.5; pointer-events: none;":""}">
        ...
        ${z.length>0?F`
          <div class="section" style="${["dry","auto","fan_only"].includes(l)||C?"opacity: 0.5; pointer-events: none;":""}">
                  ...
                  ${N.map((t,e)=>F`
                    <div class="notch-wrapper">
                      <button
                        ...
                        @click=${()=>this._setPreset(t)}
                      ></button>
                      <span class="notch-label ${e===j?"current":""}">${0===e?"N":mt(t)}</span>
                    </div>
                  `)}
                </div>
              </div>
        ...
        ${f||b||y?F`
          <div class="section">
            ...
            <div class="toggles">
              ${f?F`
                <div class="toggle-card" style="${C?"opacity: 0.5; pointer-events: none;":""}"
      ...
      <ha-card style="${c}" class="gh-full-card">
        <div class="gh-header">
          ...
          <div style="display: flex; gap: 8px;">
            ...
            `:""}
            <button class="gh-power-btn ${i?"on":""}" ?disabled=${A} style="${A?"opacity: 0.5; pointer-events: none;":""}" @click=${e=>this._togglePower(t)}>
              <ha-icon icon="mdi:power"></ha-icon>
            </button>
          </div>
        </div>
        ...
        <div class="gh-center">
          ...
          <div class="gh-subtitle-large">
            <div>${m}</div>
            ${S?F`<div style="font-size: 1rem; opacity: 0.7; margin-top: 4px;">${S}</div>`:""}
          </div>
        </div>

        <div class="gh-action-row" style="${A?"opacity: 0.5; pointer-events: none;":""}">
          <button class="gh-circular-btn" ?disabled=${!i||"fan_only"===n} @click=${t=>{t.stopPropagation(),this._adjustTemp(-.5,s,r)}}>
            <ha-icon icon="mdi:minus"></ha-icon>
          </button>
          <div style="width: 48px;"></div>
          <button class="gh-circular-btn" ?disabled=${!i||"fan_only"===n} @click=${t=>{t.stopPropagation(),this._adjustTemp(.5,s,a)}}>
            <ha-icon icon="mdi:plus"></ha-icon>
          </button>
        </div>

        <div class="gh-select-container" style="${A?"opacity: 0.5; pointer-events: none;":""}">
          <!-- Mode Dropdown -->
          ...
          <!-- Preset Dropdown -->
          <div class="gh-select-wrapper" style="${i&&"fan_only"!==n?"":"opacity: 0.5; pointer-events: none;"}">
            ...
            ${"preset"===this._ghDropdown?F`
              <div class="gh-dropdown-menu">
                ${u.map(e=>{const i=t.attributes.preset_mode===e||"none"===e&&(!t.attributes.preset_mode||/^cv[\s_]/.test(t.attributes.preset_mode));return F`
          ...
          ${v.length>0?F`
            <div class="gh-select-wrapper" style="${!i||["dry","auto","fan_only"].includes(n)?"opacity: 0.5; pointer-events: none;":""}">
              ...
              ${"cv"===this._ghDropdown?F`
                <div class="gh-dropdown-menu">
                  ${v.map(e=>{const i=mt(e),s=t.attributes.preset_mode===e||0===i&&(!t.attributes.preset_mode||!/^cv[\s_]/.test(t.attributes.preset_mode));return F`
        ...
        ${_||f||b||$||x?F`
          <div class="gh-extra-chips">
            ${_?F`<div class="gh-chip ${"on"===_.state?"active":""}" style="${A?"opacity: 0.5; pointer-events: none;":""}" @click=${()=>this._toggleSwitch(l.nanoe_switch,_.state)}><ha-icon icon="mdi:virus-outline"></ha-icon>Nanoe</div>`:""}
            ${f?F`<div class="gh-chip ${"on"===f.state?"active":""}" style="${A?"opacity: 0.5; pointer-events: none;":""}" @click=${()=>this._toggleSwitch(l.display_switch,f.state)}><ha-icon icon="mdi:lightbulb-outline"></ha-icon>Display</div>`:""}
            ${b||y?F`<div class="gh-chip ${"on"===y?.state?"active":""}" @click=${()=>b?this._pressButton(l.coil_clean_button):null}><ha-icon icon="mdi:spray"></ha-icon>${"on"===y?.state?"Cleaning...":"Clean Coil"}</div>`:""}
            ${$?F`<div class="gh-chip-text"><ha-icon icon="mdi:lightning-bolt"></ha-icon>Today: ${ut($.state)} kWh</div>`:""}
            ${x?F`<div class="gh-chip-text"><ha-icon icon="mdi:lightning-bolt"></ha-icon>Yesterday: ${ut(x.state)} kWh</div>`:""}
          </div>
        `:""}
        
        ${w?F`
          <div class="gh-footer-text">
            <ha-icon icon="mdi:wifi"></ha-icon> ${w.state} dBm
          </div>
        `:""}
      </ha-card>
    `}_renderCompact(t,e,i,s,o,n,r,a,c){"unavailable"!==t.state&&t.state;const l=i?"fan_only"===n?"FA":null!=s?`${s}°`:"--":"Off",d="on"===this.hass.states[this._config.coil_cleaning_sensor]?.state,p=t.attributes.preset_mode;let h=[];if(i&&(h.push(this._modeLabel(n)),p&&"none"!==p))if(/^cv[\s_]/.test(p)){const t=mt(p);h.push(0===t?"Normal":t+"%")}else h.push(p.charAt(0).toUpperCase()+p.slice(1));const m=h.length?h.join(" • "):"";return F`
      <ha-card style="${c}" class="compact-card" @click=${()=>{this._haptic("selection"),this._expanded=!0}}>
        ...
        <div class="compact-footer">
          ...
          <div class="compact-subtitle" style="display: flex; flex-direction: column; align-items: center; justify-content: center; line-height: 1.2;">
            ...
            ${m?F`<div style="font-size: 0.75rem; opacity: 0.7;">${m}</div>`:""}
          </div>
    ...
    `}getCardSize(){return"compact"!==this._config?.layout||this._expanded?5:2}};t([dt({attribute:!1})],gt.prototype,"hass",void 0),t([pt()],gt.prototype,"_config",void 0),t([pt()],gt.prototype,"_openPanel",void 0),t([pt()],gt.prototype,"_expanded",void 0),t([pt()],gt.prototype,"_ghDropdown",void 0),gt=t([(t=>(e,i)=>{void 0!==i?i.addInitializer(()=>{customElements.define(t,e)}):customElements.define(t,e)})("miraie-ac-card-in")],gt);export{gt as MirAIeACCard};
